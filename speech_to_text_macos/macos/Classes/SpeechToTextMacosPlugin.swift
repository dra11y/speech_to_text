...
public class SpeechToTextMacosPlugin: NSObject, FlutterPlugin {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        os_log("handle call", log: pluginLog, type: .error)
        switch call.method {
...
//            && self.audioSession.recordPermission == AVAudioSession.RecordPermission.granted
    ...
    private func initialize(_ result: @escaping FlutterResult) {
        ...
        case SFSpeechRecognizerAuthorizationStatus.denied:
            os_log("Permission permanently denied", log: pluginLog, type: .info)
            sendBoolResult(false, result)
        case SFSpeechRecognizerAuthorizationStatus.restricted:
            os_log("Device restriction prevented initialize", log: pluginLog, type: .info)
            sendBoolResult(false, result)
        default:
            os_log("Has permissions continuing with setup", log: pluginLog, type: .debug)
    ...
    fileprivate func loadSound(_ assetPath: String) -> AVAudioPlayer? {
        ...
        if let soundUrl = flutterBundle.url(forResource: assetPath, withExtension: nil) {
            do {
...
extension SpeechToTextMacosPlugin: AVAudioPlayerDelegate {
    public func audioPlayerDidFinishPlaying(_: AVAudioPlayer,
                                            successfully _: Bool)
    {
        if let playEnd = onPlayEnd {
            playEnd()
        }
    }
}
